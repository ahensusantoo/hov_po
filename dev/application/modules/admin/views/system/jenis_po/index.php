                ...
                <div class="card-body">
                    ...
                    <form action="" id="MyForm" method="post" autocomplete="off" accept-charset="utf-8">
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="group-input" id="notifikasi_kode_jenis_po">
                                    <label for="kode_jenis_po">Kode Jenis_po</label>
                                    <input type="text" name="kode_jenis_po" id="kode_jenis_po" class="form-control form-control-sm" placeholder="Kode Jenis_po" disabled/>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="group-input" id="notifikasi_nama_jenis_po">
                                    <label for="nama_jenis_po">Nama Jenis_po</label>
                                    <input type="text" name="nama_jenis_po" id="nama_jenis_po" class="form-control form-control-sm" placeholder="Nama Jenis_po" disabled/>
                                </div>
...
<script>
            ...
            success:function(data){
                ...
                if(data.record.length > 0 ){
                    ...
                    $.each(data.record, function( key, value ) {
                        html += `
                            <tr data-id_record="`+value.id_jenis_po+`" class="table_edit">
                                <td>${ no ++ }</td>
                                <td>`+value.nama_jenis_po+`</td>
                                <td>`+value.kode_jenis_po+`</td>
                                <td class="text-center">`+badge(value.stts_aktif_jenis_po == 1 ? 'success' : 'danger', value.stts_aktif_jenis_po == 1 ? 'aktif' : 'non aktif')+`</td>
                            </tr>
                        `
                    });
    ...
    $("#tbl_list").on('click', '.table_edit', function() {
        if (clickTimer === null) {
            ...
            $('#id').val(detail.record.id_jenis_po);
            $('#nama_jenis_po').val(detail.record.nama_jenis_po);
            $('#kode_jenis_po').val(detail.record.kode_jenis_po);
            $('#stts_aktif').prop('checked', parseInt(detail.record.stts_aktif_jenis_po));
